...
from icmp_poller import ICMPConfig, PingStatus, PingResult, ICMPPoller, RTTAnalyzer
...
class TestPingResult:
    ...
    def test_error_result(self):
        ...
        result = PingResult(
            is_alive=False, status=PingStatus.ERROR, error="Network unreachable"
        )
    ...
    async def test_async_ping_multiple_concurrent(self):
        ...
        with patch.object(poller, "async_ping", new_callable=AsyncMock) as mock_async:
            mock_async.side_effect = [
                ...
                PingResult(
                    is_alive=True, status=PingStatus.SUCCESS, response_time_ms=20.0
                ),
                PingResult(
                    is_alive=False, status=PingStatus.TIMEOUT, response_time_ms=None
                ),
...
class TestRTTAnalyzer:
    ...
    def test_detect_anomalies_custom_threshold(self):
        ...
        anomalies_strict = RTTAnalyzer.detect_anomalies(
            rtt_values, threshold_multiplier=2.0
        )

        # Lenient threshold (5.0 std dev)
        anomalies_lenient = RTTAnalyzer.detect_anomalies(
            rtt_values, threshold_multiplier=5.0
        )
...
class TestCriticalRTTCapture:
    ...
    def test_rtt_captured_in_result(self):
        ...
        result = PingResult(
            is_alive=True, status=PingStatus.SUCCESS, response_time_ms=15.5
        )
    ...
    def test_rtt_preserved_in_dict(self):
        ...
        result = PingResult(
            is_alive=True, status=PingStatus.SUCCESS, response_time_ms=20.3
        )
