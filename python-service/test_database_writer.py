...
from database_writer import (
    DatabaseConfig, DiscoveryQueueWriter,
    DeviceStatusWriter, DeviceMonitor
)
...
class TestDiscoveryQueueWriter:
    ...
    def test_write_discovery_offline_device(self):
        ...
        with patch.object(writer, 'get_connection') as mock_conn:
            ...
            assert data['discovery_status'] == 'pending'  # Still pending for AutoAssignment
    ...
    def test_schema_column_names_exact_match(self):
        ...
        with patch.object(writer, 'get_connection') as mock_conn:
            ...
            required_columns = [
                'ip_address', 'is_alive', 'snmp_available', 'sys_descr',
                'sys_name', 'sys_contact', 'sys_object_id', 'sys_location',
                'discovered_at', 'discovery_status', 'error_message'
            ]
...
class TestDeviceStatusWriter:
    ...
    def test_update_device_status_no_change(self):
        ...
        with patch.object(writer, 'get_connection') as mock_conn:
            ...
            mock_cursor.fetchone.return_value = {'is_alive': True, 'snmp_available': False}
            mock_conn.return_value.__enter__.return_value.cursor.return_value.__enter__.return_value = mock_cursor
            ...
            assert not any("INSERT INTO device_status_history" in sql for sql in sql_calls)
    ...
    def test_update_device_status_with_snmp(self):
        ...
        with patch.object(writer, 'get_connection') as mock_conn:
            mock_cursor = MagicMock()
            mock_cursor.fetchone.return_value = {'is_alive': False, 'snmp_available': False}
            mock_conn.return_value.__enter__.return_value.cursor.return_value.__enter__.return_value = mock_cursor
            
            writer.update_device_status(1, ping_result, snmp_result)
            
            # Verify snmp_available updated
            update_calls = [call[0] for call in mock_cursor.execute.call_args_list if "UPDATE device" in call[0]]
    ...
    def test_update_device_last_seen_online(self):
        ...
        ping_result = PingResult(is_alive=True, status=PingStatus.SUCCESS, response_time_ms=12.0)
    ...
    def test_schema_device_columns_exact_match(self):
        ...
        with patch.object(writer, 'get_connection') as mock_conn:
            ...
            # Get UPDATE call
            update_call = next(call for call in mock_cursor.execute.call_args_list if "UPDATE device" in call[0][0])
            sql = update_call[0][0]
    ...
    def test_schema_history_columns_exact_match(self):
        ...
        with patch.object(writer, 'get_connection') as mock_conn:
            ...
            history_call = next(call for call in mock_cursor.execute.call_args_list 
    ...
    def test_rtt_not_written_to_database(self):
        ...
        with patch.object(writer, 'get_connection') as mock_conn:
            ...
            all_data = [call[0][1] if len(call[0]) > 1 else {} for call in mock_cursor.execute.call_args_list]
...
class TestDeviceMonitor:
    ...
    def test_discover_device(self):
        ...
        ping_result = PingResult(is_alive=True, status=PingStatus.SUCCESS, response_time_ms=10.0)
    ...
    def test_monitor_device(self):
        ...
        ping_result = PingResult(is_alive=True, status=PingStatus.SUCCESS, response_time_ms=12.5)
...
class TestCriticalSchemaValidation:
    ...
    def test_discovery_queue_columns_documented(self):
        ...
        # Schema: 2025_01_15_000000_create_discovery_queue_table.php
        expected_columns = {
            'id', 'ip_address', 'is_alive', 'snmp_available',
            'sys_descr', 'sys_name', 'sys_contact', 'sys_object_id',
            'sys_location', 'discovered_at', 'discovery_status',
    ...
    def test_device_status_history_columns_documented(self):
        ...
        # Schema: 2025_08_28_072103_create_device_status_history_table.php
        expected_columns = {
            'id', 'device_id', 'status', 'changed_at',
