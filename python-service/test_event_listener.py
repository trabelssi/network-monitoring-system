...
async def test_snmp_trap_receiver_init(mock_db_config):
    ...
    receiver = SNMPTrapReceiver(host="0.0.0.0", port=162, db_config=mock_db_config)
...
async def test_snmp_trap_handle_event_device_found(mock_db_config):
    ...
    with patch.object(receiver.status_writer, "get_device_id_by_ip", return_value=42):
        with patch.object(
            receiver.status_writer, "update_device_status", return_value=True
        ):
            with patch.object(
                receiver.api_client, "send_device_event", new_callable=AsyncMock
            ) as mock_api:
                event = DeviceEvent(
                    device_id=None,
                    ip_address="192.168.1.10",
...
async def test_syslog_listener_init(mock_db_config):
    ...
    listener = SyslogListener(host="0.0.0.0", port=514, db_config=mock_db_config)
...
def test_syslog_classify_interface_down():
    ...
    assert (
        listener._classify_syslog_event("interface GigabitEthernet0/1 down")
        == "interface_down"
    )
...
def test_syslog_classify_interface_up():
    ...
    assert (
        listener._classify_syslog_event("interface FastEthernet0/1 up")
        == "interface_up"
    )
...
def test_syslog_classify_reboot():
    ...
    assert (
        listener._classify_syslog_event("SYS-5-RELOAD: Reload requested")
        == "device_reboot"
    )
    assert (
        listener._classify_syslog_event("System restart completed") == "device_reboot"
    )
...
def test_syslog_classify_config_change():
    ...
    assert (
        listener._classify_syslog_event("SYS-5-CONFIG_I: Configuration saved")
        == "config_change"
    )
...
def test_syslog_classify_error():
    ...
    assert (
        listener._classify_syslog_event("CRIT-1-ERROR: Critical error detected")
        == "device_error"
    )
...
async def test_syslog_handle_event_device_found(mock_db_config):
    ...
    with patch.object(listener.status_writer, "get_device_id_by_ip", return_value=100):
        with patch.object(
            listener.status_writer, "update_device_status", return_value=True
        ):
            with patch.object(
                listener.api_client, "send_device_event", new_callable=AsyncMock
            ) as mock_api:
                event = DeviceEvent(
                    device_id=None,
...
async def test_event_listener_manager_init_all_enabled(mock_db_config):
    ...
    manager = EventListenerManager(
        enable_traps=True, enable_syslog=True, db_config=mock_db_config
    )
...
async def test_event_listener_manager_init_traps_only(mock_db_config):
    ...
    manager = EventListenerManager(
        enable_traps=True, enable_syslog=False, db_config=mock_db_config
    )
...
async def test_event_listener_manager_init_syslog_only(mock_db_config):
    ...
    manager = EventListenerManager(
        enable_traps=False, enable_syslog=True, db_config=mock_db_config
    )
...
async def test_event_listener_manager_init_none_enabled(mock_db_config):
    ...
    manager = EventListenerManager(
        enable_traps=False, enable_syslog=False, db_config=mock_db_config
    )
