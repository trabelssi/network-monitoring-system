...
class LaravelAPIClient:
    ...
    def __init__(
        self, base_url: str = "http://php:80", api_token: Optional[str] = None
    ):
        """
        ...
        if not self.api_token:
            logger.warning(
                "INTERNAL_API_TOKEN not set - API calls will fail authentication"
            )
...
class SNMPTrapReceiver:
    ...
    async def handle_event(self, event: DeviceEvent):
        ...
        if device_id is None:
            logger.warning(
                f"Device not found for IP {event.ip_address}, skipping event"
            )
            return
...
class EventListenerManager:
    ...
    def __init__(
        ...
        self.trap_receiver = (
            SNMPTrapReceiver(db_config=db_config) if enable_traps else None
        )
        self.syslog_listener = (
            SyslogListener(db_config=db_config) if enable_syslog else None
        )
