...
class DiscoveryQueueWriter:
        ...
        try:
            with connection.cursor() as cursor:
                ...
                data = {
                    ...
                    'error_message': ping_result.error or (snmp_result.error if snmp_result else None),
    ...
    def _get_id_by_ip(self, cursor, ip_address: str) -> int:
        ...
        cursor.execute("SELECT id FROM discovery_queue WHERE ip_address = %s", (ip_address,))
...
class DeviceStatusWriter:
            ...
            with connection.cursor() as cursor:
                ...
                if status_changed:
                    self._record_status_change(
                        cursor,
                        device_id,
                        'online' if current_is_alive else 'offline'
                    )
    ...
    def get_device_id_by_ip(self, ip_address: str) -> Optional[int]:
        ...
        try:
            with connection.cursor() as cursor:
                cursor.execute("SELECT id FROM device WHERE ip_address = %s", (ip_address,))
                result = cursor.fetchone()
